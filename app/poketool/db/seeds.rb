...
type = ActiveSupport::JSON.decode(File.read('/usr/src/app/poketool/db/types.json'))
type.each do |data|
  Type.create(name:data['name'], screen_name:data['screen_name'])
end

personality = ActiveSupport::JSON.decode(File.read('/usr/src/app/poketool/db/personalities.json'))
